...
| [workshop-genai-product-canvas](./agentic-workloads/workshop-genai-product-canvas) | Hands-on companion to `genai-product-canvas`: the same canvas, then 90 minutes actually building what it describes on Amazon Bedrock AgentCore. A wildlife-monitoring agent is designed on paper, run locally, pointed at remote tools through an **AgentCore Gateway**, then promoted to **AgentCore Runtime** unchanged — so the cost and latency of each canvas decision show up as real numbers. Ships the tool backend, a browser Code Editor with Claude Code on Bedrock, and every dataset embedded in the CloudFormation template, so there is nothing to pre-upload. |
