 ...
 *  - pendingAction 컨펌 카드(회의록 수정 / SFDC 로깅) — [승인]/[취소]
 *  - 상단 MCP(SFDC) 연결 상태 + 미연결 시 연결 버튼
 *
...
function AgentChatPanel({ meetingId, enabled, hasMeetingRecord }: AgentChatPanelProps) {
  ...
  // MCP 상태 폴링(최초 1회) — SFDC 로깅 가능 여부 표시.
  useEffect(() => {
  ...
  return (
    <div className="qm-right-panel-card">
      <div className="qm-right-panel-card-header qm-agent-header">
        <h4>💬 회의록 어시스턴트</h4>
        <span
          className={`qm-agent-sfdc-badge qm-agent-sfdc-badge--${mcpStatus === 'connected' ? 'on' : 'off'}`}
          title={mcpStatus === 'connected' ? 'SFDC 연결됨' : 'SFDC 미연결'}
        >
          <span className="qm-agent-sfdc-dot" />
          SFDC
        </span>
      </div>
        ...
        {!enabled ? (
          ...
          <>
            ...
            <div className="qm-agent-messages" ref={scrollRef}>
              ...
              {messages.length === 0 && hasMeetingRecord && (
                <div className="qm-empty-state qm-right-panel-empty">
                  예: "액션 아이템에 'X 팔로업' 추가해줘", "이 회의를 SFDC tech activity로 기록해줘"
                </div>
              )}
              {messages.map((m, i) => (
            ...
            <div className="qm-agent-input">
              <textarea
                ...
                placeholder="회의록 수정·SFDC 기록을 요청하세요 (Enter 전송)"
                rows={2}
                disabled={isSending}
              />
