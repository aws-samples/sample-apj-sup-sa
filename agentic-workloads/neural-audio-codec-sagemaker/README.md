...
Both codecs run on CPU; no GPU is required. This sample uses `ml.c5.2xlarge` (x86-64,
8 vCPU, 16 GiB) because a single worker loads both codecs into memory at once and 16 GiB
gives comfortable headroom. Smaller instances can work for one codec or lower
concurrency; the code is not architecture-specific, but the pinned AWS Deep Learning
Container image is x86-64, so use an x86 instance family unless you rebuild for Graviton.
