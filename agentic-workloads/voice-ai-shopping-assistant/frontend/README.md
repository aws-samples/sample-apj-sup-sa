# Silo 4 — Frontend (Vite + React + TS) — "Aisle" voice grocery assistant

Voice-first SPA on **S3 + CloudFront**. Voice-orb hero with tabs for the data panels.

> **Targets the DEPLOYED agent, not the frozen v2 spec.** The live `summit_agent`
> runtime (the Aisle grocery agent — the name is historical from the
> Fargate→AgentCore migration) speaks an older **v1 contract over Daily/WebRTC**
> (not AgentCore `/ws`).
> See `src/types/contracts.live.ts` for the real shapes. The frozen
> `src/types/contracts.ts` (v2) is left untouched; a thin adapter
> (`lib/event-adapter.ts`) isolates the transport so a future v2 `/ws` agent can be
...
```bash
npm install
npm run dev          # http://localhost:5173 — connects to the live hackathon endpoint by default
npm run build        # → dist/
npm run typecheck
```
Default endpoint baked in `src/config.ts`; override with `VITE_SESSION_API_URL`.
Connecting joins the shared Daily room `isaac-aws.daily.co/voice-ai-hackathon`
(single room — fine for one presenter; concurrent users would collide).

## Layout
```
src/
  types/contracts.ts        # FROZEN v2 (untouched)
  types/contracts.live.ts   # actual deployed v1 shapes
  ...
  lib/event-adapter.ts      # v1 app-message → store (transport isolation)
