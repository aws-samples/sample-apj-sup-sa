...
AgentCore Runtime**. The shopper talks to Aisle to search live Woolworths
inventory, keep a running cart and a persistent grocery list, hear what's on
...
The agent calls the live **AgentCore Gateway** for all grocery actions (real
Woolworths catalog + cart/order in Aurora) and uses **AgentCore Memory** to
...
| Tool | What it does |
|---|---|
| `search_products` | Search live Woolworths inventory (brand, price, size, allergens, dietary tags, specials) |
...
#    NOT a public Function URL, which was removed as a Sev2). Set DAILY_ROOM_URL
...
> **Security note:** a public Lambda Function URL (`AuthType=NONE`) was previously used for `/start` and flagged as a Sev2. It has been removed and replaced with an API Gateway REST API. Do **not** re-create a Lambda Function URL — `lambda/deploy.sh` deliberately doesn't, and deletes one if found. Any future CDK `ApiStack` must expose `/start` via API Gateway (or Function URL with IAM auth), never a public Function URL.
...
- **Production hardening**: API Gateway method auth is currently `NONE`; lock it to the CloudFront origin (CORS/auth), and consider a per-session Daily room + meeting token instead of one shared room. Never expose `/start` via a public Lambda Function URL (Sev2).
