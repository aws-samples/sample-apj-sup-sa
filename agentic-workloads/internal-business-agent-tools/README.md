...
| DB access guardrail | READ-ONLY DB user (SELECT only on app tables) + keyword denylist |
...
- The MCP server connects to the database using a dedicated READ-ONLY PostgreSQL user (`readonly_user`) that is granted `SELECT` only on application tables (`customers`, `internal_projects`). This prevents DDL and DML operations at the database level regardless of the SQL generated by the LLM. The `query_database` tool additionally applies a keyword denylist as a defense-in-depth measure, though this alone can be bypassed.
- There is currently no differentiation of access levels by end user — all requests share the same read-only database credentials.
